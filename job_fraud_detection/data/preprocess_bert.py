...
def main(input_path, output_dir):
    ...
    def clean_and_mark(row):
        return (
            ...
            # f"[E_TYPE] {strip_html(row['employment_type'])} "
            # f"[LOC] {strip_html(row['location'].split(',')[0])} "
            # f"[DEP] {strip_html(row['department'])} "
            # f"[IND] {strip_html(row['industry'])} "
    ...
    df = df.drop(columns=[
        "job_id", "title", "description", "location", "department", "company_profile", "requirements",
    ...
    X_train, X_test, y_train, y_test = train_test_split(df["text"], df["fraudulent"], test_size=0.3, random_state=42, stratify=df["fraudulent"])
